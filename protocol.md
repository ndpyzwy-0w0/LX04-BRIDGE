...
`VIDEO` 是电脑某一块显示器的 JPEG 画面，缩放成 800×480 后投到音箱。音箱从右侧菜单选「屏幕镜像」开始投屏，选「状态监视」回到 CPU / GPU 页面。上位机「同步屏幕」下拉框选择投哪一块显示器。`mirror_info` 只把当前显示器名称告诉音箱，画在画面顶部。JPEG 通常约 15–25KB，约 20 帧/秒（USB 忙时自动丢旧帧只发最新画面），和音频共用同一条 USB 隧道。payload 上限 **256KB**。
