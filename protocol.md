...
`VIDEO` 走单独的 **17891** 通道，不和音频/音量/STATUS 挤在 17890 上。画面是 800×480 JPEG，音箱从右侧菜单选「屏幕镜像」开始投屏，选「状态监视」立刻停止抓屏。上位机「同步屏幕」下拉框选择投哪一块显示器。`mirror_info` 仍走 17890，只把显示器名称告诉音箱。音箱在 17891 上每收到一帧回 `VIDEO_ACK`，电脑等确认后再发下一帧，避免画面隧道里堆旧图。JPEG 大约 8–16KB。payload 上限 **256KB**。投屏时上位机暂停把电脑声音送进音箱，避免和画面抢 USB。
