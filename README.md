...
- 上位机「同步系统弹窗」打开后，Windows 右下角原生提示（例如 Cursor 申请权限、任务完成）会放大显示在音箱屏幕上，点按即可点电脑上的按钮；点弹窗外区域关闭该提示
...
默认端口：`17890`（音频/音量/状态），镜像另走 `17891`。上位机执行 `adb forward` 这两路后分别连接，不依赖 Wi-Fi。
